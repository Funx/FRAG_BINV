...
function timeareaGetter(date) {
  var area = -1 * date.getTimeareaOffset();
  var paddedarea = (area >= 0) ? "+" : "";

  paddedarea += padNumber(Math[area > 0 ? 'floor' : 'ceil'](area / 60), 2) +
                padNumber(Math.abs(area % 60), 2);
 ...
 *   * `'a'`: am/pm marker
 *   * `'Z'`: 4 digit (+sign) representation of the timearea offset (-1200-+1200)
 ...
 *    shorter versions like yyyy-MM-ddTHH:mmZ, yyyy-MM-dd or yyyyMMddTHHmmssZ). If no timearea is
 *    specified in the string input, the time is considered to be in the local timearea.
